...
for ax, subtype_name in zip(axes.ravel(), subtypes):
    ...
    for i, group in enumerate(display_order):
        ...
        mean = values.mean()
        sd = values.std(ddof=1)
        # Percentages cannot be negative; clip only the displayed lower SD bound.
        lower = mean - max(0.0, mean - sd)
        ax.errorbar(i, mean, yerr=[[lower], [sd]], color="black", capsize=3, lw=1.1)
    ...
    label = label.replace("typeR_bone_remodel", "Type R / bone-remodeling")
    label = label.replace("venous_ACKR1", "Venous / ACKR1")
    label = label.replace("lymphatic", "Lymphatic")
    ax.set_title(label)
